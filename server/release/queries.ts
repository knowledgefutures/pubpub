...
import { getPubDraft, getPubDraftDoc, getStepsBetweenVersions } from 'server/utils/firebaseAdmin';
...
const getStepsSinceLastRelease = async (
	...
	return getStepsBetweenVersions(draftId, previousHistoryKey, currentHistoryKey, editorSchema);
};
...
const createDiscussionAnchorsForRelease = async (
	...
	const steps = await getStepsSinceLastRelease(draft.id, previousRelease, currentHistoryKey);
	...
	// no commits in the range -- try stored stepMaps from checkpoint
	const pgCheckpoint = await getDraftCheckpoint(draft.id, sequelizeTransaction);
	...
	if (pgCheckpoint.stepMapToKey < currentHistoryKey) {
		const additionalSteps = await getStepsBetweenVersions(
			draft.id,
			pgCheckpoint.stepMapToKey,
			currentHistoryKey,
			editorSchema,
		);

		const additionalFlatSteps = additionalSteps.reduce((a, b) => [...a, ...b], []);
		const newRanges = additionalFlatSteps.map((s) =>
